...
class TCU:
    ...
    def get_control_sensor_temp(self):
        """
        R → XX.XXXX C$ — temperature control sensor in °C (4 decimal places).
        This is the PT500 sensor used by the PID controller.
        May read a different physical location than the M command (measuring sensor).
        Returns float or None on failure.
        """
        raw = self._send('R')
        try:
            return float(raw.replace('C', '').replace('$', '').strip())
        except (ValueError, AttributeError):
            return None
