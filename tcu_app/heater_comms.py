# =============================================================================
# heater_comms.py — Heater Modbus RTU Communication
# =============================================================================
...
class HeaterComms:
    ...
    Usage:
        hc = HeaterComms()
        ok = hc.connect()
        ok = hc.set_watts(5000)
        w  = hc.read_actual_watts()
        hc.disconnect()
    """
    ...
    def connect(self) -> bool:
        ...
        if not port:
            print("HeaterComms: heater_port not configured")
            return False
        try:
            ...
            print(f"HeaterComms: connected on {port} baud={baud} slave={slave_id}")
            return True
        except Exception as e:
            print(f"HeaterComms: connect failed — {e}")
            self._connected = False
            return False
    ...
    def set_watts(self, watts: int) -> bool:
        ...
        if not isinstance(watts, int):
            print(f"HeaterComms.set_watts: expected int, got {type(watts)}")
            return False
        if watts < 0 or watts > HEATER_MAX_WATTS:
            print(f"HeaterComms.set_watts: {watts}W out of range [0, {HEATER_MAX_WATTS}]")
            return False
    ...
    def _write_register(self, reg: int, value: int) -> bool:
        ...
        if not self.is_connected():
            print("HeaterComms: not connected")
            return False
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                print(f"HeaterComms: write reg {reg:#04x} attempt {attempt+1} failed — {e}")
                time.sleep(_RETRY_DELAY_S)
        return False

    def _read_register(self, reg: int):
        """Read integer value from Modbus register with retries. Returns None on failure."""
        if not self.is_connected():
            return None
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                print(f"HeaterComms: read reg {reg:#04x} attempt {attempt+1} failed — {e}")
                time.sleep(_RETRY_DELAY_S)
        return None
